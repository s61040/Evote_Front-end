...
import Eventaddrole from "./Eventaddrole";
...
function EventDetail2() {
  ...
  const Check_hash = () => {
    Axios.post(`${window._env_.API_URL}/count_check`, {
      ...
      console.log(" = ", check_H);
      // Axios.post("http://localhost:3001/Check_hash", {
      //       id : check_H[2].id,
      //       vote : check_H[2].Vote,
  ...
  return (
    <div>
      <Navmas head={event_name} role={login} />
          ...
          <Row>
            ...
            <Col>
              <Button
                ...
                disabled={0}
                onClick={() => Detail()}
                href="/MainAdmin/Event/Eventstat/detail/detail2/detail3"
                style={{ margintop: 50, width: "95%" }}
              >
                หน้าต่อไป
              </Button>{" "}
